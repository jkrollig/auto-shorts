...
def get_available_filename(base_filename, extension):
    """
    Returns an available filename by incrementing a counter if the base filename already exists.
    """
    if not os.path.exists(base_filename + extension):
        return base_filename + extension
    
    counter = 1
    while os.path.exists(f"{base_filename}_{counter:03d}{extension}"):
        counter += 1
    
    return f"{base_filename}{counter:03d}{extension}"

def synthesize_speech(text, number):
...
def combine_audio_video(number):
    ...
    input_audio = f"mp3/output_{number:03}.mp3"
    ...
    try:
        ...
        trimmed_audio_file = f"trimmed/trimmed_audio_{number:03}.mp3"
        trimmed_video_file = f"trimmed/trimmed_video_{number:03}.mp4"
...
def generate_video_from_answer(text, number):
    print("Text-to-Speech begins")
    synthesize_speech(text,number)
    print("Text-to-Speech ends")
    return combine_audio_video(number)
