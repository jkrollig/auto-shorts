...
print("MODEL LOADED")
def transcribe_with_word_timestamps(audio_file, output_json, transcript):
...
def create_animated_subtitles(input_video, json_file, output_video, keyword_images, subtitle_data):
    # Read JSON file
    ...
    video = (
        ...
        .filter('subtitles', subtitle_file, force_style='Fontname=Impact,FontSize=36,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,Bold=1,BorderStyle=1,Outline=1,Shadow=0,Alignment=10')
    )
    ...
    if audio_stream:
        ...
        output = ffmpeg.output(video, audio, output_video,
              vcodec='libx264',     # H.264 using CPU
...
def executor(number, transcript):
    input_mp4 = f'./withaudio/xd_{number:03}.mp4'
    output_json = f'./jasons/transcript_{number:03}.json'
    audio_file = f'./mp3/output_{number:03}.mp3'
    subtitle_data = transcribe_with_word_timestamps(audio_file, output_json, transcript)
    # Usage
    ...
    output_video = f'./final_videos/video_{number:03}.mp4'

    create_animated_subtitles(input_mp4, output_json, output_video, keyword_to_image, subtitle_data=subtitle_data)
